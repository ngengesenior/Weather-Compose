...
/*@Composable
fun DayDetailUI(navController: NavController){
    DayDetailUI(viewModel = hiltViewModel(), navController = navController)
}*/
...
fun DayDetailUI(viewModel: WeatherViewModel,navController: NavController){
    ...
    if (dailyWeather != null && timeZoneOffset.value != null) {
        ...
        Log.d("AppTag", "DayDetailUI: $dailyWeather $timeZoneOffset")
...
fun DayDetailUI(weather: DailyWeather,timeZoneOffset:Long) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Row {
            Column {
                BoldText(text = weather.weather[0].main)
